...
function signupPage(enabled) {
  ...
  return documentShell("회원가입", `<main class="auth-page"><form id="signupForm" class="auth-card"><div class="brand"><span class="brand-dot"></span>Jorip Monitor</div><div class="eyebrow">CREATE ACCOUNT</div><h1>회원가입</h1><p>새 계정은 관찰자 권한으로 시작합니다.</p><label>아이디<input name="username" type="text" autocomplete="username" minlength="3" maxlength="40" pattern="[A-Za-z0-9._-]+" required></label><label>비밀번호<input name="password" type="password" autocomplete="new-password" minlength="8" maxlength="128" required></label><label>비밀번호 확인<input name="password_confirmation" type="password" autocomplete="new-password" minlength="8" maxlength="128" required></label><button class="primary">계정 만들기</button><div id="error" class="error" role="alert"></div><div class="actions"><a class="button" href="/login">로그인으로 돌아가기</a></div></form></main><script>${formScript("signupForm", "/api/signup", true)}</script>`);
}

function authSettingsPage(enabled) {
  return documentShell("가입 설정", `<main class="auth-page"><form id="settingsForm" class="auth-card"><div class="brand"><span class="brand-dot"></span>Jorip Monitor</div><div class="eyebrow">MEMBERSHIP</div><h1>가입 설정</h1><p>공개 회원가입 여부를 관리합니다. 새 가입자는 안전한 관찰자 권한으로 시작합니다.</p><label class="switch"><input id="publicSignup" type="checkbox" ${enabled ? "checked" : ""}><span>공개 회원가입 허용<br><small class="meta">누구나 회원가입 화면에서 계정을 만들 수 있습니다.</small></span></label><button class="primary">설정 저장</button><div id="message" class="error" role="status"></div><div class="actions"><a class="button" href="/users">사용자 관리로 돌아가기</a></div></form></main><script>document.querySelector("#settingsForm").onsubmit=async function(event){event.preventDefault();var message=document.querySelector("#message");message.textContent="";try{var response=await fetch("/api/settings/signup",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({public_signup_enabled:document.querySelector("#publicSignup").checked})});var data=await response.json();if(!response.ok)throw Error(data.message||"저장하지 못했습니다.");message.style.color="#2f6e4f";message.textContent="가입 설정을 저장했습니다."}catch(error){message.style.color="#b9403a";message.textContent=error.message}}</script>`);
}

function messagePage(title, message, href, action) {
  return documentShell(title, `<main class="auth-page"><section class="auth-card"><div class="brand"><span class="brand-dot"></span>Jorip Monitor</div><div class="eyebrow">ACCOUNT</div><h1>${title}</h1><p>${message}</p><div class="actions"><a class="button" href="${href}">${action}</a></div></section></main>`);
}
